...
beatAML_incidence_matrix <- plotInput(
  ...
  verbose = T # save the figures to local folder
  )
```
...
# show the igraph bipartGraph object
bipartGraph
```
#### 1.2.2 interactive:
Function **plotBipartiteInteractive()** will print the interactive bipartite graph based on [visNetwork](https://datastorm-open.github.io/visNetwork/)
package.
...
```
### 1.3 Analysis of the global network properties
```{r}
...
sub_matrices <- extractSubMatrix(
  ...
  plot_weight = T,
  verbose = T,
  print_skim = F # just to reduce the length of vignette
  )
```
...
## 3 Find the cluster in the projected unipartite graphs
