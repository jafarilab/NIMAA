...
Then, using a variety of different network projection methods, two unipartite graphs are constructed on the given submatrix. NIMAA provides several options for clustering projected networks and selecting the best one based on external prior knowledge. When weighted bipartite networks are considered, the best clustering results are used as the benchmark for imputation analysis. This benchmark is used to figure out which imputation method is the best. It looks at how similar the clustering results are before and after the imputations. By using imputation analysis, we tried to get more information from the whole dataset even though there were some missing values.
```{r setup}
library(NIMAA)
```
...
The `plotCluster()` function makes an interactive network figure that shows nodes that belong to the same group in the same color and nodes that belong to different groups in separate colors.
...
Based on the `fpc` package [@fpc], and @DBLP, the `scoreCluster` function provides additional internal cluster validity measures such as entropy and coverage. The concept of scoring is according to the weight fraction of all intra-cluster edges relative to the total weight of all edges in the graph.
...
The `imputeMissingValue()` function utilizes the given imputation methods to impute missing values in the input data matrix. The output is a list with each element representing an updated version of input data matrix with no missing values using a separate imputation method. The following is a detailed list of imputation techniques:
