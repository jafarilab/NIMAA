...
#' \dontrun{
#'   data <- NIMAA::beatAML
#'   inc_mat <- NIMAA::el2IncMatrix(data,print_skim = F)
#'   sub_matrices <- extractSubMatrix(inc_mat,print_skim = F)
#' }
extractSubMatrix <- function(x,
  ...
  req_shape <- c("Square", "Rectangular_row",
  ...
  plotSubmatrix(x=t_x,print_dataframe = print_dataframe_t,rect_data = rect_data_t,
                verbose = verbose,palette = palette,
...
findElementMaxSubmatrix <- function(x) {
  ...
  pos_first_na <- apply(X = x,
                        MARGIN = 1,
                        function(row){min(which(is.na(row)))})
...
plotSubmatrix <- function(x,print_dataframe,rect_data,
  ...
  p <- ggplot2::ggplot(data = print_dataframe,
